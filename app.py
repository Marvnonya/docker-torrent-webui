...
def generate_screenshots(video_path, output_base_path, mode, quality):
    ...
    try:
        ...
        if mode == 'grid':
            ...
            if os.path.exists(output_jpg): result_file = output_jpg; preview_data = output_jpg
            ...
            for i in range(1, steps):
                ...
                cmd.append(img_path)
                subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                if os.path.exists(img_path) and os.path.getsize(img_path) > 0: image_list.append(img_path)
...
def index():
    ...
    mediainfo_content = ""; error_msg = None
